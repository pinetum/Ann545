...
void MainFrame::drawResults(cv::Mat weights, cv::Mat &img, wxString msg)
{
    for(int r =0; r< weights.rows; r++)
    {
        for(int c = 0; c< weights.cols; c++)
        {
            ...
            pt1.x=weights.at<cv::Vec2d>(r, c)[0];
            pt1.y=weights.at<cv::Vec2d>(r, c)[1];
            
            if(r!=weights.rows-1)
            {
                pt2.x=weights.at<cv::Vec2d>(r+1, c)[0];
                pt2.y=img.rows - weights.at<cv::Vec2d>(r+1, c)[1];
                cv::line(img, pt1, pt2, cv::Scalar(255));
                
            }
            if(c!=weights.cols-1)
            {
                pt3.x=weights.at<cv::Vec2d>(r, c+1)[0];
                pt3.y=img.rows - weights.at<cv::Vec2d>(r, c+1)[1];
